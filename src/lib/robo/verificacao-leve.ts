...
export async function executarVerificacaoLeve(): Promise<ResultadoVerificacaoLeve> {
  ...
  for (const { job, intervaloMin, toleranciaMin } of JOBS_MONITORADOS) {
    ...
    if (estaAtrasado(ultima, agora, intervaloMin, toleranciaMin)) {
      achados.push(`Job "${job}" atrasado (última execução: ${ultima?.toISOString() ?? 'nunca'}).`)
      const resultado = await dispararCatchUp(job)
      if (resultado.ok) {
        correcoes.push(`Disparei "${job}" manualmente como reforço (catch-up).`)
      } else {
        achados.push(`Não consegui disparar "${job}" de reforço: ${resultado.erro}`)
      }
    }
  }
  ...
  for (const p of travados) {
    achados.push(`Pedido ${p.numero} travado em ${p.status} desde ${p.createdAt.toISOString()} (mais de 48h).`)
  }
  ...
  for (const log of comErro) {
    achados.push(`E-mail "${log.tipo}" falhou para ${log.destinatario}.`)
    if (log.certificadoId) {
      await prisma.emailLog.delete({ where: { id: log.id } })
      correcoes.push(`Removi o registro de falha do e-mail "${log.tipo}" (${log.destinatario}) — o robô de e-mails vai tentar de novo no próximo ciclo.`)
    }
  }
