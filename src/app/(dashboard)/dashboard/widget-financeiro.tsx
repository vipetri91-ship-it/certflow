...
export function WidgetFinanceiro({ aReceber, aReceberVencidos, aReceberQtd, aReceberVencidosQtd }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden" style={{ height: '100%' }}>
      ...
      <div className="flex-1 flex flex-col justify-between px-5 pb-5 gap-2 min-h-0">

        {/* Total em aberto — destaque principal */}
        <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center">
          <p className="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Total em Aberto</p>
          <p className="text-3xl font-black text-green-700 dark:text-green-400">{fmt(aReceber)}</p>
        ...
        <Link href="/financeiro/contas-a-receber?status=VENCIDO"
          ...
          <div className="flex items-center gap-2">
            <AlertTriangle className={`w-4 h-4 shrink-0 ${temVencidos ? 'text-red-500' : 'text-gray-300 dark:text-slate-600'}`} />
            <div>
          ...
          <p className={`text-lg font-black ${temVencidos ? 'text-red-600 dark:text-red-400' : 'text-gray-300 dark:text-slate-600'}`}>
        ...
        <div className="bg-teal-50 dark:bg-teal-900/20 rounded-xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <DollarSign className="w-4 h-4 text-teal-500 shrink-0" />
            <div>
          ...
          <p className="text-lg font-black text-teal-600 dark:text-teal-400">{fmt(emDia)}</p>
        </div>
